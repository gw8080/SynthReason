...
void setup()
{
  ...
  if (mode.equals("nlp") == true) {
    ...
    int NLPconstructionAttempts = split(join(loadStrings("sim.txt"), "\n"), " ").length-1;
    ...
    for (int q = 0; q < mainLoops; q++ ) {
      ...
      for (int h = 0; h < NLPconstructionAttempts; ) {
        ...
        if (NLPFunction.equals("generate") == true) {
          ...
          for (int h2 = 0; h2 < NLPconstructionAttempts && exit == false; h2++ ) {
            String combo1 = words( split(simulationData, " ")[h], split(res, " "), vocab[G1]);
            if (combo1.length() > 3) {
              if (h < NLPconstructionAttempts) {
                h++;
              }
              for (int h3 = 0; h3 < NLPconstructionAttempts && exit == false; h3++ ) {
                String combo2 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[G2], vocab[G3]);
                if (combo2.length() > 3) {
                  if (h < NLPconstructionAttempts) {
                    h++;
                  }
                  for (int h4 = 0; h4 < NLPconstructionAttempts && exit == false; h4++ ) {
                    String combo3 = words( split(simulationData, " ")[h], split(res, " "), vocab[G4]);
                    if (combo3.length() > 3) {
                      if (h < NLPconstructionAttempts) {
                        h++;
                      }
                      for (int h5 = 0; h5 < NLPconstructionAttempts && exit == false; h5++ ) {
                        String combo4 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[G5], vocab[G6]);
                        if (combo4.length() > 3) {
                          if (h < NLPconstructionAttempts) {
                            h++;
                          }
                          for (int h6 = 0; h6 < NLPconstructionAttempts && exit == false; h6++ ) {
                            String combo5 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[G7], vocab[G8]);
                            if (combo5.length() > 3) {
                              if (h < NLPconstructionAttempts) {
                                h++;
                              }
        ...
        if (NLPFunction.equals("recall") == true) {
          ...
          boolean exit = false;
          for (int h2 = 0; h2 < NLPconstructionAttempts && exit == false; h2++ ) {
            String combo1 = words(split(simulationData, " ")[h], split(res, " "), vocab[int(split(cat[randCat], ",")[0])]);
            if (combo1.length() > 3) {
              if (h < NLPconstructionAttempts) {
                h++;
              }
              for (int h3 = 0; h3 < NLPconstructionAttempts && exit == false; h3++ ) {
                String combo2 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[int(split(cat[randCat], ",")[1])], vocab[int(split(cat[randCat], ",")[2])]);
                if (combo2.length() > 3) {
                  if (h < NLPconstructionAttempts) {
                    h++;
                  }
                  for (int h4 = 0; h4 < NLPconstructionAttempts && exit == false; h4++ ) {
                    String combo3 = words(split(simulationData, " ")[h], split(res, " "), vocab[int(split(cat[randCat], ",")[3])]);
                    if (combo3.length() > 3) {
                      if (h < NLPconstructionAttempts) {
                        h++;
                      }
                      for (int h5 = 0; h5 < NLPconstructionAttempts && exit == false; h5++ ) {
                        String combo4 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[int(split(cat[randCat], ",")[4])], vocab[int(split(cat[randCat], ",")[5])]);
                        if (combo4.length() > 3) {
                          if (h < NLPconstructionAttempts) {
                            h++;
                          }
                          for (int h6 = 0; h6 < NLPconstructionAttempts && exit == false; h6++ ) {
                            String combo5 = wordsMulti( split(simulationData, " ")[h], split(res, " "), vocab[int(split(cat[randCat], ",")[6])], vocab[int(split(cat[randCat], ",")[7])]);
                            if (combo5.length() > 3) {
                              if (h < NLPconstructionAttempts) {
                                h++;
                              }
      ...
      if (selfEnhancement == true) {
        simulationData = stream.replace(".\n", " ");
        NLPconstructionAttempts = split(simulationData, " ").length-1;
      }
...
String createSimulation()
{
  ...
  for (int x = 0; x < iterations; ) {
    if (x == 0) {
      ...
      currentState = split(split(split(simulationData, " ")[round(random(split(simulationData, " ").length-1))], ":")[1], "->")[0];
    ...
    if (simulationData.indexOf("\n") > -1) {
      ...
      if (function.length-1 == 2) {
        ...
        if (currentState.length() > 2) {
          ...
          stream += currentState + " " + function[2] + " ";
          status.flush();
        }
        if (retry >= retryLimit) {
...
String[] selectFunction(String simulationData, String currentState) {
  ...
  for (int x = 0; x < functionChances; x++) {
    int match = round(random(split(simulationData, " ").length-1));
    if (split(simulationData, " ")[match].indexOf(":") > -1 && split(simulationData, " ")[match].indexOf("->") > -1) {
      if (split(split(split(simulationData, " ")[match], ":")[1], "->")[0].equals(currentState) == true && split(split(split(simulationData, " ")[match], ":")[1], "->")[1].length() > 3) {
        state = split(split(simulationData, " ")[match], ":")[0] + ":" + split(split(split(simulationData, " ")[match], ":")[1], "->")[1] + ":" + split(split(split(simulationData, " ")[match], ":")[1], "->")[0];
        break;
      }
    }
  }
